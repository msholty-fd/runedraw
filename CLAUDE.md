...
runedraw/
  ...
  Views/
    ...
    Components/             — HeroPortraitView, CardView, GearListView, etc.
```
...
### Equipment (FAB + Commander hybrid)

Equipment is a subtype of `Card` (`equipmentSlot != nil`). Every equipment card has:
- **`statBonus`** — passive bonus always active while equipped (ATK, DEF, HP, etc.)
- **`effect: CardEffect`** — the activated ability triggered in combat
- **`description: String`** — the activated ability text (e.g. "Activate: Deal 8 damage.")
- **`activatedCost: Int`** — energy cost to activate (0 = free)

**Two ways to use equipment:**
1. **Equip to slot** (weapon/offhand/helm/chest/boots/ring/amulet): passive bonus is always on; activated ability can be triggered once per turn from `CombatView`'s equipment strip. `GameEngine.usedEquipmentActivations: Set<EquipmentSlot>` resets each turn.
2. **Add to deck** (`engine.addEquipmentToDeck(_:)`): moves from `hero.inventory` to `hero.cardCollection`. When drawn and played from hand, fires the same `effect` once then discards normally. No passive bonus (slot is empty).

Equipment is found as loot drops (goes to `hero.inventory` / `GearBag`). From EquipmentView bag panel: EQUIP to slot, TO DECK to convert, or DROP to discard permanently.

**Build identity (Commander-style):** Unique items are designed so their passive + activated ability together define an archetype. E.g. "Warlord's Visage" (+6 ATK, +20 HP, +4 Life on Kill / Activate: +4 Strength this combat) signals a Barbarian Strength-stacking build. Players build their deck around what their equipment rewards.

`engine.activateEquipment(_ slot:)` applies the equipment's `effect` (same damage/block/poison/draw logic as card play). `engine.canActivateEquipment(_ slot:)` checks cost, unused this turn, and non-empty effect.

Equipment is generated by `LootDatabase`: common/magic/rare from `BaseItemDatabase` + `AffixDatabase`; uniques from `UniqueItemDatabase`. All base items have activated abilities baked in. `ItemSize` is still stored on `Card` for save compat but is no longer used in any UI.
