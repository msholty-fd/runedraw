...
struct LootDatabase {
    ...
    /// Generates `count` loot items — combat cards only (equipment system removed).
    ...
    static func generateLoot(floorNumber: Int, isBoss: Bool, count: Int,
        ...
        var items: [Card] = []
        var attempts = 0
        while items.count < count && attempts < count * 20 {
            attempts += 1
            let rarity = rates.roll()
            // All drops are now combat cards
            if let hc = heroClass {
                if let card = CardDatabase.droppableCard(for: hc, rarity: rarity) {
                    items.append(card)
                }
            } else if let hc = HeroClass.allCases.randomElement(),
                      let card = CardDatabase.droppableCard(for: hc, rarity: rarity) {
                items.append(card)
            }
        }
        return items
